...
class ObjectCollectionIterator implements \Iterator, \ArrayAccess, \Countable
{
    ...
    public function sort($fieldname, $direction = "ASC")
    {
        ...
        if(!$value = $object->getUnformatted($fieldname)){
            throw new Exceptions\FlatbedException("Cannot sort by '$fieldname' no data by that name can be found in {$this}.");
        }
    ...
    public function index($x)
    {
        ...
        if(!count($this->collection)){
            throw new Exceptions\FlatbedException("$this->className is empty, cannot retrieve index($x)");
        }
