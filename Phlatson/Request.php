...
class Request
{
    ...
    public function __construct()
    {
        ...
        $this->httpUrl = $this->scheme . "://{$this->hostname}{$this->get('url')}";

        $this->ssl = !empty($_SERVER['HTTPS']) && 'on' == strtolower($_SERVER['HTTPS']);
        // find the client ip
        $this->ip = $_SERVER['HTTP_X_FORWARDED_FOR'] ?? null;
        $this->ip = null === $this->ip && isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : null;
