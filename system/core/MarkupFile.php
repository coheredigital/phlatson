...
class MarkupFile extends Core
{
    ...
    public function setFilename($filename)
    {
        ...
        if (!file_exists($filename)) {
            throw new WireException("Template file does not exist: '$filename'");
        }
    ...
    public function setGlobal($name, $value, $overwrite = true)
    {
        ...
        if (!$overwrite && isset(self::$globals[$name])) {
            return;
        }
    ...
    public function ___render()
    {
        ...
        $fuel = array_merge(
            $this->getArray(),
            self::$globals
        ); // so that script can foreach all vars to see what's there

        extract($fuel);
        ob_start();
        if ($this->prependFilename) {
            require($this->prependFilename);
        }
        require($this->filename);
        if ($this->appendFilename) {
            require($this->appendFilename);
        }
        $out = "\n" . ob_get_contents() . "\n";
        ob_end_clean();
