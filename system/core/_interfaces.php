...
/**
 * For classes that contain lookup items, as used by XSaveableItemsLookup
 *
 */
interface HasLookupItems {

	/**
	 * Get all lookup items, usually in a XArray derived type, but specified by class
	 *
	 */
 ...
 *
 * See the X class that provides an example implementation of all these.
 *
 */
interface XHookable {

	/**
	 * Provides the gateway for calling hooks in ProcessX
	 *
	 ...
	 *
	 * See the X::runHooks() method for the full implementation of hook calls.
	 *
	 * @param string $method
	...
	/**
	 * Provides the implementation for calling hooks in ProcessX
	 *
...
interface XFuelable {
	...
	/**
	 * Get or inject a ProcessX API variable
	 *
	 * 1. As a getter (option 1):
	 * Usage: $this->wire('name'); // name is an API variable name
	 * If 'name' does not exist, a XException will be thrown.
	 *
...
interface XNoticeable {
	 ...
	 *
	 * Fatal errors should still throw a XException (or class derived from it)
	 *
	 * @param string $text
	 * @param int $flags See Notices::flags
	 * @return $this
...
/**
 * Interface for ProcessX database layer
 *
 */
